function [x0s,s,sv,idout,ll,k,k0,dif, nbimag1]=cokri(x,x0,model,c,itype,avg,block,nd,ival,nk,rad,ntok)
%
...
% casesen off;
%nbimag=0;
x0s=[];s=[];sv=[];id=[];l=[];k=[];k0=[];dif=[];ll=[];nbimag1=[];
...
[x0s,s,id,l,k,k0,dif,~]=cokri2(t,grid,[],model,c,sv,99,avg,ng);
...
for i=1:ntok:m
   ...
   if ival>=1,
      ...
      for ip=1:np:p,
         ...
         [x0ss,ss,idout,l,k,k0,dif,nbimag]=cokri2(t,t2,id,model,c,sv,itype,avg,ng);
         est(ip:ip+np-1)=x0ss(ip:ip+np-1);
         sest(ip:ip+np-1)=ss(ip:ip+np-1);
%         diff(ip:ip+np-1)=dif(ip:ip+np-1);
  ...
  %  dif=[dif;[t2,diff]];
       ...
       if ~isempty(t)
         ...
         [x0ss,ss,idout,l,k,k0,diff,nbimag]=cokri2(t,t2,id,model,c,sv,itype,avg,ng);
         x0s=[x0s;[x0(i:i+nnx-1,:),x0ss]];
         s=[s;[x0(i:i+nnx-1,:),ss]];
         nbimag1=[nbimag1;nbimag];
%         ll=[ll,l];
    %     dif=[dif;[x0(i:i+nnx-1,:),diff]];
     else
         x0s=[x0s;[x0(i:i+nnx-1,:),nan*ones(1,p)]];
         s=[s;[x0(i:i+nnx-1,:),nan*ones(1,p)]];    
     end
   end
%keyboard
end
